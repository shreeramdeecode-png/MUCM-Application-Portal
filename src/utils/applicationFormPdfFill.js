...
function markEnglishProficiency(page, value, font) {
  ...
  else if (v) drawInSlot(page, value, PAGE2_FIELDS.otherLanguages, font, BODY)
}
...
function markHowHeard(page, value, otherText, font) {
  ...
  else if (v) drawInSlot(page, otherText || value, PAGE4_FIELDS.howHeardOther, font, BODY)
}
...
export async function fillRedesignedApplicationPdf(pdfDoc, formValues, options = {}) {
  ...
  drawMultilineInSlot(p1, permanent, PAGE1_FIELDS.permanentAddress, helvetica, BODY, 2)

  if (!v.sameAsPermanent) {
    const mailing = joinParts(
      v.mailingAddress,
      v.mailingCity,
    ...
    drawMultilineInSlot(p1, mailing, PAGE1_FIELDS.mailingAddress, helvetica, BODY, 2)
  }
  ...
  if (asText(v.contactAddress)) {
    drawMultilineInSlot(p1, v.contactAddress, PAGE1_FIELDS.emergencyAddress, helvetica, SMALL, 2)
  }

  // Page 2
  drawInSlot(p2, v.fatherName, PAGE2_FIELDS.fatherName, helvetica, BODY)
  drawInSlot(p2, v.fatherEmail, PAGE2_FIELDS.fatherEmail, helvetica, BODY)
  drawInSlot(p2, v.fatherPhone, PAGE2_FIELDS.fatherPhone, helvetica, BODY)
  drawInSlot(p2, v.motherName, PAGE2_FIELDS.motherName, helvetica, BODY)
  drawInSlot(p2, v.motherEmail, PAGE2_FIELDS.motherEmail, helvetica, BODY)
  drawInSlot(p2, v.motherPhone, PAGE2_FIELDS.motherPhone, helvetica, BODY)
  ...
  if (disciplineExplain) {
    drawMultilineInSlot(p3, disciplineExplain, PAGE3_FIELDS.disciplineExplain, helvetica, BODY, 4)
  }
  ...
  if (disabilityExplain) {
    drawMultilineInSlot(p3, disabilityExplain, PAGE3_FIELDS.disabilityExplain, helvetica, BODY, 3)
  }
